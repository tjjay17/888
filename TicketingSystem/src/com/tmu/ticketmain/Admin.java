...
public class Admin extends User {
    ...
    //the user, type and credit Formatter functions help to correlate a username/type/credit to fit the format of the files

    public static String userFormatter(String username) {
        String tempFormattedUser = username;
        String formattedUser;
        int length = tempFormattedUser.length();
        do {
    ...
    public List<String> getUserList(List<User> storedList) {
        //basic
        String entry;
        //method specific
        String username;
    ...
    public List<String> getRefundRequests(List<RefundRequest> refundRequestList) {
        //Basic requirements
        int i = 0;
        String sellerName;
        String buyerName;
...
//        int position = 0;
        ...
        while (createUser_active) {
            ...
            //User type
            System.out.println("Enter the two letter user type: aa - admin, fs - full standard, bs - buy standard, ss - sell standard");
            do {
...
//                            System.out.println("Not found, removing request");
                            ...
                            while (refundList.size() > 0) {
                                ...
                                switch (prompt) {
                                    case "yes":
                                        ...
                                        CentralCore.addRefundTransaction(05, refundRequestList.get(i).getRefundAmount(), refundRequestList.get(i).getBuyerName(), refundRequestList.get(i).getSellerName());
                ...
                case "new refund":
                    ...
                    check1 = getUser(seller, userList) && !CentralCore.getUserType(seller).toLowerCase().equals("bs");
                    if (check1 == true) {
                        sellerPos = getPosition(seller, userList);
                    }else{
                        System.out.println("Wrong user type or user does not exist - exiting refund.");
                        break;
                    }
                    //find buyer
                    ...
                    check2 = getUser(buyer, userList) && !CentralCore.getUserType(buyer).toLowerCase().equals("ss");
                    if (check2 == true) {
...
//-----------------------------------------------------------------------------------------------------------------------------------
                    ...
                    sellerCredit = tempSeller;
                    storedList.get(sellerPos).setCredit(sellerCredit);
                    CentralCore.addRefundTransaction(05, refundCredit, buyer, seller);
                    //confirmation message
